...
- Name: admin\referral-engine.spec.ts >> 5-Layer Referral & Earnings Engine >> ARLY-29: 'Earnings by Position' summary totals match the sum of that position's actual records
- Location: tests\admin\referral-engine.spec.ts:435:7
...
Locator: locator('div').filter({ hasText: /^Nearest$/ }).first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('div').filter({ hasText: /^Nearest$/ }).first()
