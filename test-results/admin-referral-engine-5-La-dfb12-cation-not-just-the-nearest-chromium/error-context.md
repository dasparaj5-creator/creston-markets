...
- Name: admin\referral-engine.spec.ts >> 5-Layer Referral & Earnings Engine >> ARLY-24: every position in a multi-layer event receives its own notification, not just the nearest
- Location: tests\admin\referral-engine.spec.ts:372:7

# Error details

```
Error: Test-C should have a notification, not just the nearest upline member
...
Call log:
  - Test-C should have a notification, not just the nearest upline member with timeout 10000ms
...
- complementary:
  ...
  - navigation:
    ...
    - link "Support":
      ...
      - text: Support
- banner:
  - paragraph: Welcome back, Test Chain C
  - button "Toggle theme":
    - img
  - button:
    - img
