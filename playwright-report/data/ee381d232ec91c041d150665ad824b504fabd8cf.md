...
- Name: admin\referral-engine.spec.ts >> 5-Layer Referral & Earnings Engine >> 5.3 first settlement establishes a baseline, pays nothing yet
- Location: tests\admin\referral-engine.spec.ts:188:7

# Error details

```
Error: locator.getAttribute: Error: strict mode violation: locator('select').first().locator('option').filter({ hasText: /Test Chain F/i }) resolved to 41 elements:
