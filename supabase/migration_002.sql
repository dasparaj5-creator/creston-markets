...
-- Adds: multi-document KYC support, plans unique constraint, and updates
-- the portfolio_snapshots RLS policy so admin-verified account statements
-- are visible to clients (per confirmed Phase 1 operating model).
-- ============================================================================

-- 1. Fix missing unique constraint on plans.name (needed for seed upserts)
alter table plans add constraint plans_name_unique unique (name);

-- 2. New enums for multi-document KYC
do $$ begin
...
-- 4. Update portfolio_snapshots RLS: clients now see their own snapshots
